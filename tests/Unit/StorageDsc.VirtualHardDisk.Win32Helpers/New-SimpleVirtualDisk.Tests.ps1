<#
    .SYNOPSIS
        Unit test for New-SimpleVirtualDisk.
#>
...
Describe 'StorageDsc.VirtualHardDisk.Win32Helpers\New-SimpleVirtualDisk' {
    Context 'Creating and attaching a new virtual disk (vhdx) successfully' {
        BeforeAll {
            Mock -CommandName New-VirtualDiskUsingWin32 -MockWith { 0 }
        ...
        It 'Should not throw an exception' {
            InModuleScope -ScriptBlock {
                ...
                { New-SimpleVirtualDisk @testParams } | Should -Not -Throw
            }

            Should -Invoke -CommandName New-VirtualDiskUsingWin32 -Exactly -Times 1 -Scope It
    ...
    Context 'Creating a new virtual disk failed due to exception' {
        BeforeAll {
            Mock -CommandName New-VirtualDiskUsingWin32 -MockWith { 5 }
        }
        ...
        It 'Should throw an exception in creation method' {
            InModuleScope -ScriptBlock {
                ...
                ($script:localizedData.CreateVirtualDiskError -f $testParams.VirtualDiskPath, $win32Error.Message),
                    $win32Error)

                { New-SimpleVirtualDisk @testParams } | Should -Throw -ExpectedMessage $exception.Message
            }
