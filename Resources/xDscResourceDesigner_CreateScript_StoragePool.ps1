...
$modulename = 'xStorage'
$Description = 'This module is used to create a StoragePool. Functionality is limited to create, add a disk to existing, a rename of existing and COMPLETE destroy. It can only filter on disk size and/or numberofdisks'
...
if (!(test-path (join-path $modules $modulename))) {
    ...
    $P += New-xDscResourceProperty -Name DriveSize -Type Uint32 -Attribute Write -Description "Specifies the size of the disk (in GB) to filter on in the 'canpool' collection."
    ...
    New-xDscResource -Name StoragePool -Property $P -FriendlyName StoragePool @standard
}
