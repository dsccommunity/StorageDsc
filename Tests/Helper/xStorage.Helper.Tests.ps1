...
Describe 'xStorage whole of module tests' {
    ...
    Context -Name "Validate example files" {
        ...
        It "Should compile MOFs for all examples correctly" {
            ...
            {
                Copy-item -Path "$env:APPVEYOR_BUILD_FOLDER\Modules\xStorage" `
                          -Destination 'C:\WINDOWS\system32\WindowsPowerShell\v1.0\Modules\xStorage' `
                          -Recurse
            }

            $examplesWithErrors = 0
            Get-ChildItem -Path "$RepoRoot\Modules\xStorage\Examples" -Filter "*.ps1" -Recurse | ForEach-Object -Process {
                    $path = $_.FullName
                    try
                    {
                        . $path
            ...
            if ($env:APPVEYOR -eq $true)
            {
                Remove-item -Path 'C:\WINDOWS\system32\WindowsPowerShell\v1.0\Modules\xStorage' `
                            -Recurse -Force -Confirm:$false
                # Restore the load of the module to ensure future tests have access to it
                Import-Module -Name "$RepoRoot\Modules\xStorage\xStorage.psd1" -Global -Force
            }
        }
